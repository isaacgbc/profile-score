...
async function main() {
  ...
  // Archive v1 rewrite prompts (v2 is now active)
  const rewriteKeys = ["rewrite.linkedin.section", "rewrite.cv.section"];
  for (const key of rewriteKeys) {
    ...
    if (updated.count > 0) {
      console.log(`  Archived: ${key} v1 (${updated.count} rows)`);
    }
  }
